class MuslimTest {
    ...
    generateStory(totalScore, categoryScores) {
        ...
        if (strengths.length > 0) {
            ...
            story += `<span class="highlight">${topStrength.category}</span> konusundaki verileri iyi biliyorsun (%${topStrength.percentage}). `;
        }
        
        // Zayıf yönler
        if (weaknesses.length > 0) {
            const mainWeakness = weaknesses[0];
            story += `Ancak <span class="warning">${mainWeakness.category}</span> konusunda eksiklerin var (%${mainWeakness.percentage}). `;
            
            if (mainWeakness.category === "Ekonomi") {
                story += 'Ekonomik rakamları takip etmek önemli çünkü günlük hayatınızı doğrudan etkiliyor. ';
            } else if (mainWeakness.category === "Alım Gücü") {
                story += 'Alım gücündeki değişimleri bilmek, bütçe planlaması için kritik. ';
            } else if (mainWeakness.category === "Demokrasi") {
                story += 'Demokrasi endekslerini bilmek, ülkenin dünya üzerindeki konumunu anlamak için önemli. ';
            }
        }
        ...
        story += '<br><br>';
        if (totalScore >= 75) {
            story += 'Türkiye\'yi iyi tanıyorsun. Bu bilgilerle bilinçli kararlar alabilirsin. 📊';
        } else if (totalScore >= 50) {
            story += 'Bazı gerçekleri biliyorsun ama daha fazla araştırmalısın. Bilgi güçtür! 🔍';
        } else {
            story += 'Resmi verileri takip etmeye başla. Gerçekleri öğrenmek ilk adım. 📚';
        }
    ...
    getScoreMessage(score) {
        if (score >= 90) {
            return "Türkiye'nin son 20 yılını çok iyi biliyorsun! 📊";
        } else if (score >= 75) {
            return "Ekonomik ve sosyal verileri iyi takip ediyorsun 📈";
        } else if (score >= 60) {
            return "Genel bir fikrin var ama detaylar eksik 📚";
        } else if (score >= 45) {
            return "Birçok gelişmeden habersiz kalmışsın 📉";
        } else if (score >= 30) {
            return "Türkiye'nin gerçeklerini daha fazla araştırmalısın 🔍";
        } else {
            return "Ülkenin ekonomik ve sosyal durumunu öğrenmen gerekiyor 📖";
        }
    }
    ...
    shareWhatsApp() {
        ...
        const text = `🇹🇷 "Türkiye'nin 20 Yılı" testinde %${score} doğru yaptım! Sen Türkiye'yi ne kadar tanıyorsun?`;
